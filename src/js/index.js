  ...
  var app = {
    ...
    initTpl: function () {
      ...
      if (urlId == 9) {
        ...
        jQ('body').addClass('not-recommend');
      }
    ...
    NavRender: function () {
      ...
      function HttpNav() {
        COVER.$post(COVER.apis().getAllChannels, {}).then(function (data) {
          // console.log('getAllChannels() => data: ', JSON.stringify(data.list));
          NavHtml(data);
        })
      }
      ...
      function NavHtml(data) {
        // console.log(data);
        // 高亮导航条(首页)
        juicer.register('isNav', function (id, type) {
          if (!COVER.isEmpty(urlId) && urlId == id) {
            return 'tc-menu-hover';
          } else if (COVER.isEmpty(urlId) && type == 1) {
            return 'tc-menu-hover';
          } else {
            return '';
          }
        });
        jQ('.tc-menu ul').html(juicer(self.navTpl(), data)).promise().then(function () {
          // 滚动导航到中间
          //var navleft = jQ('.tc-menu-hover').offset().left,
          var menuwidth = jQ('.tc-menu').width(),
            navwidth = jQ('.tc-menu li').width();
          if (jQ('.tc-menu-hover').length > 0) {
            if (jQ('.tc-menu-hover').offset().left > (menuwidth - navwidth)) {
              var moveWidth = (jQ('.tc-menu-hover').offset().left - menuwidth) + navwidth * 3;
              jQ('.tc-menu').scrollLeft(moveWidth);
            }
          } else {
            jQ('.tc-menu').scrollLeft(0);
          }
        });
      }
      this.navEvent();
    ...
    SectionRender: function () {
      ...
      return COVER.$post(COVER.apis().getList, payload).then(function (data) {
    ...
    // 导航事件监听
    navEvent: function () {
      jQ('.tc-menu-right').on('click', function () {
        var menuwidth = jQ('.tc-menu').width();
        jQ('.tc-menu').scrollLeft(menuwidth);
      });
    },
    // banner逻辑控制
    banner: function () {
    ...
    bannerTpl: function () {
      var tpl = [
        ...
        '        <div>',
        '            {@if item.subject_id}',
        '              <a class="swipe-a" href="${item.subject_id, item.flag | Link}" style="background-image:url(${item.img_url})">',
        '            {@else}',
        '          <a class="swipe-a" href="${item.news_id, item.flag | Link}" onclick="COVER.jumpToExternalLink(\'${item.flag + ("_") + item.news_id}\')" style="background-image: url(${item.img_url})">',
        '            {@/if}',
